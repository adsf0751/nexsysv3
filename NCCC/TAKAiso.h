 ...
 * Author: RussellBai
 *
 * Created on 2022年10月14日, 下午 2:47
 */
...
#define _TAKA_CHECK_BIT_MAP_(x, b)	((x) & (1U<<(b)))

#define _TAKA_NULL_TX_                  0
#define _TAKA_MAX_BIT_MAP_CNT_          40
#define _TAKA_MTI_SIZE_                 2
#define _TAKA_PCODE_SIZE_               3
#define _TAKA_TPDU_SIZE_                5
#define _TAKA_BIT_MAP_SIZE_             8
#define _TAKA_RRN_SIZE_                 12 /* RRN */
#define _TAKA_ISO_SEND_                 1536
#define _TAKA_ISO_RECV_                 1536
#define _TAKA_ISO_ASC_                  1 /* a */
#define _TAKA_ISO_BCD_                  2
#define _TAKA_ISO_NIBBLE_2_             3 /* ..nibble */
#define _TAKA_ISO_NIBBLE_3_             4 /* ...nibble */
#define _TAKA_ISO_BYTE_2_               5 /* ..ans */
#define _TAKA_ISO_BYTE_3_               6 /* ...ans */
#define _TAKA_ISO_BYTE_2_H_             7 /* ..ans */
#define _TAKA_ISO_BYTE_3_H_             8 /* ...ans */
#define _TAKA_ISO_ASC_1_                9 /* a */
#define _TAKA_ISO_BYTE_1_               10/* ..ans */

typedef struct
{
        int inFieldNum; /* Field Number */
        int (*inISOLoad)(TRANSACTION_OBJECT *, unsigned char *); /* 組 Field 的功能 */
} ISO_FIELD_TAKA_TABLE;

typedef struct
{
        int inFieldNum; /* Field Number */
        int (*inISOCheck)(TRANSACTION_OBJECT *, unsigned char *, unsigned char *); /* 檢查 Field 的功能 */
} ISO_CHECK_TAKA_TABLE;
...
} ISO_FIELD_TYPE_TAKA_TABLE;

typedef struct
{
        int inTxnID; /* 交易類別 */
        int *inBitMap; /* Bit Map */
        char szMTI[_TAKA_MTI_SIZE_ * 2 + 1]; /* Message Type */
        char szPCode[_TAKA_PCODE_SIZE_ * 2 + 1]; /* Processing Code */
} BIT_MAP_TAKA_TABLE;

typedef struct
{
        ISO_FIELD_TAKA_TABLE *srPackISO; /* 組封包的功能結構 */
        ISO_FIELD_TAKA_TABLE *srUnPackISO; /* 解封包的功能結構 */
        ISO_CHECK_TAKA_TABLE *srCheckISO; /* 檢查封包的功能結構 */
        ISO_FIELD_TYPE_TAKA_TABLE *srISOFieldType; /* ISO Field 型態結構 */
        BIT_MAP_TAKA_TABLE *srBitMap; /* Bit Map的陣列 */
...
int inTAKA_UnPack55(TRANSACTION_OBJECT *pobTran, unsigned char *uszUnPackBuf);
int inTAKA_UnPack58(TRANSACTION_OBJECT *pobTran, unsigned char *uszUnPackBuf);
int inTAKA_UnPack59(TRANSACTION_OBJECT *pobTran, unsigned char *uszUnPackBuf);
int inTAKA_UnPack60(TRANSACTION_OBJECT *pobTran, unsigned char *uszUnPackBuf);
int inTAKA_UnPack63(TRANSACTION_OBJECT *pobTran, unsigned char *uszUnPackBuf);
...
int inTAKA_GetBitMapTableIndex(ISO_TYPE_TAKA_TABLE *srISOFunc, int inBitMapCode);
int inTAKA_GetBitMapMessagegTypeField03(TRANSACTION_OBJECT *pobTran, ISO_TYPE_TAKA_TABLE *srISOFunc, int inTxnType, int *inTxnBitMap, unsigned char *uszSendBuf);
...
int inTAKA_Advice_ESC_SendRecvPacket(TRANSACTION_OBJECT *pobTran, int inAdvCnt);
int inTAKA_ProcessAdvice(TRANSACTION_OBJECT *pobTran);
int inTAKA_ProcessAdvice_ESC(TRANSACTION_OBJECT *pobTran);
...
int inTAKA_OnlineAnalyseEMV(TRANSACTION_OBJECT *pobTran);
int inTAKA_OnlineEMV_Complete(TRANSACTION_OBJECT *pobTran);
...
int inTAKA_ProcessReferral(TRANSACTION_OBJECT *pobTran);
...
int inTAKA_CUP_LogOn(TRANSACTION_OBJECT* pobTran);
int inTAKA_TMKKeyExchange(TRANSACTION_OBJECT *pobTran,unsigned char *uszUnPackBuf);
void vdTAKA_ISO_FormatDebug_DISP(TRANSACTION_OBJECT *pobTran, unsigned char *uszDebugBuf, int inSendLen);
void vdTAKA_ISO_FormatDebug_DISP_EMV(unsigned char *uszDebugBuf, int inFieldLen);
void vdTAKA_ISO_FormatDebug_DISP_58(unsigned char *uszDebugBuf, int inFieldLen);
void vdTAKA_ISO_FormatDebug_DISP_59(unsigned char *uszDebugBuf, int inFieldLen);
void vdTAKA_ISO_FormatDebug_PRINT(TRANSACTION_OBJECT *pobTran, unsigned char *uszDebugBuf, int inSendLen);
void vdTAKA_ISO_FormatDebug_PRINT_EMV(unsigned char *uszDebugBuf, int inFieldLen, unsigned char *uszBuffer, FONT_ATTRIB *srFont_Attrib, BufferHandle *srBhandle);
void vdTAKA_ISO_FormatDebug_PRINT_58(unsigned char *uszDebugBuf, int inFieldLen, unsigned char *uszBuffer, FONT_ATTRIB *srFont_Attrib, BufferHandle *srBhandle);
void vdTAKA_ISO_FormatDebug_PRINT_59(unsigned char *uszDebugBuf, int inFieldLen, unsigned char *uszBuffer, FONT_ATTRIB *srFont_Attrib, BufferHandle *srBhandle);
void vdTAKA_ISO_FormatDebug_POS_PRINT(TRANSACTION_OBJECT *pobTran, unsigned char *uszDebugBuf, int inSendLen);
void vdTAKA_ISO_FormatDebug_POS_PRINT_EMV(unsigned char *uszDebugBuf, int inFieldLen);
void vdTAKA_ISO_FormatDebug_POS_PRINT_58(unsigned char *uszDebugBuf, int inFieldLen);
void vdTAKA_ISO_FormatDebug_POS_PRINT_59(unsigned char *uszDebugBuf, int inFieldLen);

int inTAKA_OnlineAnalyseFISC(TRANSACTION_OBJECT* pobTran);
int inTAKA_FuncInquiryIssueBank(TRANSACTION_OBJECT *pobTran);