#include <string.h>
#include <stdio.h>
#include <stdlib.h>
#include <stdarg.h>
#include <ctosapi.h>
#include "../INCLUDES/Define_1.h"
#include "../INCLUDES/Define_2.h"
#include "../INCLUDES/Transaction.h"
#include "../PRINT/Print.h"
#include "../DISPLAY/Display.h"
#include "File.h"
#include "Function.h"
#include "PIT.h"

static  PIT_REC srPITRec;	/* construct PIT record */
extern  int     ginDebug;       /* Debug使用 extern */

/*
Function        :inLoadPITRec
Date&Time       :2015/8/31 下午 2:00
Describe        :讀PIT檔案，inPITRec是要讀哪一筆的紀錄，第一筆為0
*/
int inLoadPITRec(int inPITRec)
{
        unsigned long   ulFile_Handle;                          /* File Handle */
        unsigned char   *uszReadData;                           /* 放抓到的record */
        unsigned char   *uszTemp;                               /* 暫存，放整筆PIT檔案 */
        char            szPITRec[_SIZE_PIT_REC_ + 1];         /* 暫存, 放各個欄位檔案 */
        char    	szErrorMsg[_DEBUG_MESSAGE_SIZE_ + 1];   /* debug message */
        long            lnPITLength = 0;                       /* PIT總長度 */
        long            lnReadLength;                           /* 記錄每次要從PIT.dat讀多長的資料 */
        int             i, k, j = 0, inRec = 0;	                /* inRec記錄讀到第幾筆, i為目前從PIT讀到的第幾個字元, j為該record的長度, k為該欄位的第幾個字元 */
        int             inSearchResult = -1;                    /* 判斷有沒有讀到0x0D 0x0A的Flag */
       
        /* inLoadPITRec()_START */
        if (ginDebug == VS_TRUE)
        {
                inLogPrintf(AT, "----------------------------------------");
                memset(szErrorMsg, 0x00, sizeof(szErrorMsg));
                sprintf(szErrorMsg, "inLoadPITRec(%d) START!!", inPITRec);
                inLogPrintf(AT, szErrorMsg);
        }

        /* 判斷傳進來的inPITRec是否小於零 大於等於零才是正確值(防呆) */
        if (inPITRec < 0)
        {
                /* debug */
                if (ginDebug == VS_TRUE)
                {
                        memset(szErrorMsg, 0x00, sizeof(szErrorMsg));
                        sprintf(szErrorMsg, "inPITRec < 0:(index = %d) ERROR!!", inPITRec);
                        inLogPrintf(AT, szErrorMsg);
                }

                return (VS_ERROR);
        }

        /*
         * open PIT.dat file
         * open失敗時，if條件成立，關檔並回傳VS_ERROR
         */
        if (inFILE_Open(&ulFile_Handle, (unsigned char *)_PIT_FILE_NAME_) == VS_ERROR)
        {
                /* 開檔失敗 ，不用關檔 */
                /* 開檔失敗，所以回傳error */
                return (VS_ERROR);
        }

        /*
         * get data size 為了接下來動態宣告記憶體的大小需要知道多少 需先取得檔案Size
         * get size 失敗時，if條件成立，關檔並回傳VS_ERROR
         */
        lnPITLength = lnFILE_GetSize(&ulFile_Handle, (unsigned char *)_PIT_FILE_NAME_);
        
        if (lnPITLength == VS_ERROR)
        {
                /* GetSize失敗 ，關檔 */
                inFILE_Close(&ulFile_Handle);

                return (VS_ERROR);
        }

        /*
         * allocate 記憶體
         * allocate時多分配一個byte以防萬一（ex:換行符號）
         */
        uszReadData = malloc(lnPITLength + 1);
        uszTemp = malloc(lnPITLength + 1);
        /* 初始化 uszTemp uszReadData */
        memset(uszReadData, 0x00, lnPITLength + 1);
        memset(uszTemp, 0x00, lnPITLength + 1);

        /* seek 到檔案開頭 & 從檔案開頭開始read */
        if (inFILE_Seek(ulFile_Handle, 0, _SEEK_BEGIN_) == VS_SUCCESS)
        {
                lnReadLength = lnPITLength;

                for (i = 0;; ++i)
                {
                        /* 剩餘長度大於或等於1024 */
                        if (lnReadLength >= 1024)
                        {
                                if (inFILE_Read(&ulFile_Handle, &uszTemp[1024*i], 1024) == VS_SUCCESS)
                                {
                                        /* 一次讀1024 */
                                        lnReadLength -= 1024;

                                        /* 當剩餘長度剛好為1024，會剛好讀完 */
                                        if (lnReadLength == 0)
                                                break;
                                }
                                /* 讀失敗時 */
                                else
                                {
                                        /* Close檔案 */
                                        inFILE_Close(&ulFile_Handle);

                                        /* Free pointer */
                                        free(uszReadData);
                                        free(uszTemp);

                                        return (VS_ERROR);
                                }
                        }
                        /* 剩餘長度小於1024 */
                        else if (lnReadLength < 1024)
                        {
                                /* 就只讀剩餘長度 */
                                if (inFILE_Read(&ulFile_Handle, &uszTemp[1024*i], lnReadLength) == VS_SUCCESS)
                                {
                                        break;
                                }
                                /* 讀失敗時 */
                                else
                                {
                                        /* Close檔案 */
                                        inFILE_Close(&ulFile_Handle);

                                        /* Free pointer */
                                        free(uszReadData);
                                        free(uszTemp);

                                        return (VS_ERROR);
                                }
                        }
                }/* end for loop */
        }
        /* seek不成功時 */
        else
        {
                /* 關檔並回傳 */
                inFILE_Close(&ulFile_Handle);
                /* Free pointer */
                free(uszReadData);
                free(uszTemp);

                /* Seek失敗，所以回傳Error */
                return (VS_ERROR);
        }

        /*
         *抓取所需要的那筆record
         *i為目前從PIT讀到的第幾個字元
         *j為該record的長度
         */
        j = 0;
        for (i = 0; i <= lnPITLength; ++i)      /* "<=" 是為了抓到最後一個0x00 */
        {
                /* 讀完一筆record或讀到PIT的結尾時 */
                if ((uszTemp[i] == 0x0D && uszTemp[i+1] == 0x0A) || uszTemp[i] == 0x00)
                {
                        /* 只要讀到0x0D 0x0A，Flag改為1(表示讀到record的結尾) */
                        inSearchResult = 1;

                        /* 清空uszReadData */
                        memset(uszReadData, 0x00, lnPITLength + 1);
                        /* 把record從temp指定的位置截取出來放到uszReadData */
                        memcpy(&uszReadData[0], &uszTemp[i-j], j);
                        inRec++;
                        /* 因為inPIT_Rec的index從0開始，所以inPIT_Rec要+1 */
                        if (inRec == (inPITRec + 1))
                        {
                                break;
                        }

                        /* 為了跳過 0x0D 0x0A */
                        i = i + 2;
                        /* 每讀完一筆record，j就歸0 */
                        j = 0;
                }

                j ++;
        }

        /*
         * 如果沒有inPITRec的那筆資料或者整筆記錄都沒有0x0D 0x0A就回傳Error
         * 關檔、釋放記憶體並return VS_ERROR
         * 如果總record數量小於要存取Record的Index
         * 特例：有可能會遇到全文都沒有0x0D 0x0A
         */
        if (inRec < (inPITRec + 1) || inSearchResult == -1)
        {
                if (ginDebug == VS_TRUE)
                {
                        inLogPrintf(AT, "No data or Index ERROR");
                }

		/* 關檔 */
                inFILE_Close(&ulFile_Handle);
				
		/* Free pointer */
                free(uszReadData);
                free(uszTemp);

                return (VS_ERROR);
        }

        /* uszReadData沒抓到資料，關檔、釋放記憶體並return (VS_ERROR) */
        if (*uszReadData == 0x00)
        {
                if (ginDebug == VS_TRUE)
                {
                        inLogPrintf(AT, "No specific data.");
                }

                /* 關檔 */
                inFILE_Close(&ulFile_Handle);
				
                /* Free pointer */
                free(uszReadData);
                free(uszTemp);

                return (VS_ERROR);
        }

        /* 結構初始化 */
        memset(&srPITRec, 0x00, sizeof(srPITRec));
        /*
         * 以下pattern為存入PIT_Rec
         * i為PIT的第幾個字元
         * 存入PIT_Rec
         */
        i = 0;

        /* 01_繳費項目索引 */
        /* 初始化 */
        memset(szPITRec, 0x00, sizeof(szPITRec));
        k = 0;

        /* 從Record中讀欄位資料出來 */
        while (1)
        {
                szPITRec[k ++] = uszReadData[i ++];
                if (szPITRec[k - 1] == 0x2C	||
		    szPITRec[k - 1] == 0x0D	||
		    szPITRec[k - 1] == 0x0A	||
		    szPITRec[k - 1] == 0x00)
                {
                        break;
                }

                if (i > lnPITLength)
                {
                        if (ginDebug == VS_TRUE)
                        {
                                inLogPrintf(AT, "PIT unpack ERROR.");
                        }

                        /* 關檔 */
                        inFILE_Close(&ulFile_Handle);
						
                        /* Free pointer */
                        free(uszReadData);
                        free(uszTemp);

                        return (VS_ERROR);
                }
        }

        /*  該筆有資料 */
        if (szPITRec[0] != 0x2C	&&
	    szPITRec[0] != 0x0D	&&
	    szPITRec[0] != 0x0A	&&
	    szPITRec[0] != 0x00)
        {
                memcpy(&srPITRec.szPayItemIndex[0], &szPITRec[0], k - 1);
        }

        /* 02_端末機對應之快捷鍵(Hot Key)日 */
        /* 初始化 */
        memset(szPITRec, 0x00, sizeof(szPITRec));
        k = 0;

        /* 從Record中讀欄位資料出來 */
        while (1)
        {
                szPITRec[k ++] = uszReadData[i ++];
                if (szPITRec[k - 1] == 0x2C	||
		    szPITRec[k - 1] == 0x0D	||
		    szPITRec[k - 1] == 0x0A	||
		    szPITRec[k - 1] == 0x00)
                {
                        break;
                }

                if (i > lnPITLength)
                {
                        if (ginDebug == VS_TRUE)
                        {
                                inLogPrintf(AT, "PIT unpack ERROR");
                        }

                        /* 關檔 */
                        inFILE_Close(&ulFile_Handle);

                        /* Free pointer */
                        free(uszReadData);
                        free(uszTemp);

                        return (VS_ERROR);
                }
        }

        /*  該筆有資料 */
        if (szPITRec[0] != 0x2C	&&
	    szPITRec[0] != 0x0D	&&
	    szPITRec[0] != 0x0A	&&
	    szPITRec[0] != 0x00)
        {
                memcpy(&srPITRec.szPayItemKeyMap[0], &szPITRec[0], k - 1);
        }

        /* 03_繳費項目代碼 */
        /* 初始化 */
        memset(szPITRec, 0x00, sizeof(szPITRec));
        k = 0;

        /* 從Record中讀欄位資料出來 */
        while (1)
        {
                szPITRec[k ++] = uszReadData[i ++];
                if (szPITRec[k - 1] == 0x2C	||
		    szPITRec[k - 1] == 0x0D	||
		    szPITRec[k - 1] == 0x0A	||
		    szPITRec[k - 1] == 0x00)
                {
                        break;
                }

                if (i > lnPITLength)
                {
                        if (ginDebug == VS_TRUE)
                        {
                                inLogPrintf(AT, "PIT unpack ERROR");
                        }

                        /* 關檔 */
                        inFILE_Close(&ulFile_Handle);

                        /* Free pointer */
                        free(uszReadData);
                        free(uszTemp);

                        return (VS_ERROR);
                }
        }

        /*  該筆有資料 */
        if (szPITRec[0] != 0x2C	&&
	    szPITRec[0] != 0x0D	&&
	    szPITRec[0] != 0x0A	&&
	    szPITRec[0] != 0x00)
        {
                memcpy(&srPITRec.szPayItemCode[0], &szPITRec[0], k - 1);
        }

        /* 04_繳費項目說明 */
        /* 初始化 */
        memset(szPITRec, 0x00, sizeof(szPITRec));
        k = 0;

        /* 從Record中讀欄位資料出來 */
        while (1)
        {
                szPITRec[k ++] = uszReadData[i ++];
                if (szPITRec[k - 1] == 0x2C	||
		    szPITRec[k - 1] == 0x0D	||
		    szPITRec[k - 1] == 0x0A	||
		    szPITRec[k - 1] == 0x00)
                {
                        break;
                }

                if (i > lnPITLength)
                {
                        if (ginDebug == VS_TRUE)
                        {
                                inLogPrintf(AT, "PIT unpack ERROR");
                        }

                        /* 關檔 */
                        inFILE_Close(&ulFile_Handle);

                        /* Free pointer */
                        free(uszReadData);
                        free(uszTemp);

                        return (VS_ERROR);
                }
        }

        /*  該筆有資料 */
        if (szPITRec[0] != 0x2C	&&
	    szPITRec[0] != 0x0D	&&
	    szPITRec[0] != 0x0A	&&
	    szPITRec[0] != 0x00)  /* 因為是最後的欄位還要多判斷0x00 */
        {
                memcpy(&srPITRec.szPayItemScript[0], &szPITRec[0], k - 1);
        }

        /* release */
        /* 關檔 */
        inFILE_Close(&ulFile_Handle);

        /* Free pointer */
        free(uszReadData);
        free(uszTemp);

        /* inLoadPITRec() END */
        if (ginDebug == VS_TRUE)
        {
                memset(szErrorMsg, 0x00, sizeof(szErrorMsg));
                sprintf(szErrorMsg, "inLoadPITRec(%d) END!!", inPITRec);
                inLogPrintf(AT, szErrorMsg);
                inLogPrintf(AT, "----------------------------------------");
        }

        return (VS_SUCCESS);
}

/*
Function        :inSavePITRec
Date&Time       :2015/8/31 下午 2:00
Describe        :
*/
int inSavePITRec(int inPITRec)
{
        unsigned long   uldat_Handle;                           /* FILE Handle */
        unsigned long   ulbak_Handle;                           /* FILE Handle */
        int	        inPackCount = 0;                        /* uszWriteBuff_Record的index */
        int	        inRecIndex = 0;                         /* uszRead_Total_Buff的index */
        int             inPIT_Total_Rec = 0;                    /* PIT.dat的總筆數 */
        int             inRetVal;
        int             i = 0, j = 0;
        char            szErrorMsg[_DEBUG_MESSAGE_SIZE_ + 1];   /* debug message */
        long	        lnReadLength = 0;                       /* 每次要從PIT.dat讀多少byte出來 */
        long            lnPITLength = 0;                        /* PIT.dat檔案總長度 */
        unsigned char	*uszRead_Total_Buff;
        unsigned char   *uszWriteBuff_Record, *uszWriteBuff_Org;


        /* inSavePITRec()_START */
        if (ginDebug == VS_TRUE)
        {
                inLogPrintf(AT, "----------------------------------------");
                memset(szErrorMsg, 0x00, sizeof(szErrorMsg));
                sprintf(szErrorMsg, "inSavePITRec(%d)_START!!", inPITRec);
                inLogPrintf(AT, szErrorMsg);
        }

        /* 防呆先刪除PIT.bak */
        inFILE_Delete((unsigned char *)_PIT_FILE_NAME_BAK_);

        /* 新建PIT.bak檔案 */
        inRetVal = inFILE_Create(&ulbak_Handle, (unsigned char *)_PIT_FILE_NAME_BAK_);

        if (inRetVal != VS_SUCCESS)
        {
                inFILE_Close(&ulbak_Handle);

                return (VS_ERROR);
        }

        /* 開啟原檔案PIT.dat */
        inRetVal = inFILE_Open(&uldat_Handle, (unsigned char *)_PIT_FILE_NAME_);

        if (inRetVal != VS_SUCCESS)
        {
                /* PIT.dat開檔失敗 ，不用關檔PIT.dat */
                /* PIT.bak仍要關並刪除 */
                inFILE_Close(&ulbak_Handle);
                inFILE_Delete((unsigned char *)_PIT_FILE_NAME_BAK_);

                return(VS_ERROR);
        }

        /* 取得PIT.dat檔案大小 */
        lnPITLength = lnFILE_GetSize(&uldat_Handle, (unsigned char *)_PIT_FILE_NAME_);

        /* 取得檔案大小失敗 */
        if (lnPITLength == VS_ERROR)
        {
                /* PIT.bak和PIT.dat要關並刪除 */
                inFILE_Close(&ulbak_Handle);
                inFILE_Close(&uldat_Handle);
                inFILE_Delete((unsigned char *)_PIT_FILE_NAME_BAK_);

                return (VS_ERROR);
        }

        /* 組Write Record封包 */
        /* 給WriteBuff記憶體大小 */
        uszWriteBuff_Record = malloc(_SIZE_PIT_REC_ + _SIZE_PIT_COMMA_0D0A_);
        memset(uszWriteBuff_Record, 0x00, _SIZE_PIT_REC_ + _SIZE_PIT_COMMA_0D0A_); /* 如果用sizeof指標，會只set 4 byte */

        /* 存原PIT.dat的每筆Record 初始化 */
        uszWriteBuff_Org = malloc(_SIZE_PIT_REC_ + _SIZE_PIT_COMMA_0D0A_);
        memset(uszWriteBuff_Org, 0x00, _SIZE_PIT_REC_ + _SIZE_PIT_COMMA_0D0A_); /* 如果用sizeof指標，會只set 4 byte */

        /* uszRead_Total_Buff儲存PIT.dat全部資料 */
        uszRead_Total_Buff = malloc(lnPITLength + 1);
        memset(uszRead_Total_Buff, 0x00, lnPITLength + 1);

        inPackCount = 0; /* uszWriteBuff_Record的index位置 */
        /* 以下依照Structure宣告的順序依序將參數值塞入Buffer中並補上逗號 */

        /* PayItemIndex */
        memcpy(&uszWriteBuff_Record[0], &srPITRec.szPayItemIndex[0], strlen(srPITRec.szPayItemIndex));
        inPackCount += strlen(srPITRec.szPayItemIndex);
        uszWriteBuff_Record[inPackCount] = 0x2C;
        inPackCount++;

        /* PayItemKeyMap */
        memcpy(&uszWriteBuff_Record[inPackCount], &srPITRec.szPayItemKeyMap[0], strlen(srPITRec.szPayItemKeyMap));
        inPackCount += strlen(srPITRec.szPayItemKeyMap);
        uszWriteBuff_Record[inPackCount] = 0x2C;
        inPackCount++;

        /* PayItemCode */
        memcpy(&uszWriteBuff_Record[inPackCount], &srPITRec.szPayItemCode[0], strlen(srPITRec.szPayItemCode));
        inPackCount += strlen(srPITRec.szPayItemCode);
        uszWriteBuff_Record[inPackCount] = 0x2C;
        inPackCount++;

        /* PayItemScript */
        memcpy(&uszWriteBuff_Record[inPackCount], &srPITRec.szPayItemScript[0], strlen(srPITRec.szPayItemScript));
        inPackCount += strlen(srPITRec.szPayItemScript);

        /* 最後的data不用逗號 */
        /* 補上換行符號 */

        /* 0D是移至行首 */
        uszWriteBuff_Record[inPackCount] = 0x0D;
        inPackCount++;
        /* 0A是移至下一行 */
        uszWriteBuff_Record[inPackCount] = 0x0A;
        inPackCount++;

        /* 讀PIT.dat檔案存到uszRead_Total_Buff */
        if (inFILE_Seek(uldat_Handle, 0, _SEEK_BEGIN_) == VS_SUCCESS)
        {
                lnReadLength = lnPITLength;

                for (i = 0;; ++i)
                {
                        /* 剩餘長度大於或等於1024 */
                        if (lnReadLength >= 1024)
                        {
                                if (inFILE_Read(&uldat_Handle, &uszRead_Total_Buff[1024 * i], 1024) == VS_SUCCESS)
                                {
                                        /* 一次讀1024 */
                                        lnReadLength -= 1024;

                                        /* 當剩餘長度剛好為1024，會剛好讀完 */
                                        if (lnReadLength == 0)
                                                break;
                                }
                                /* 讀失敗時 */
                                else
                                {
                                        /* Close檔案 */
                                        inFILE_Close(&uldat_Handle);
                                        inFILE_Close(&ulbak_Handle);
                                        inFILE_Delete((unsigned char *)_PIT_FILE_NAME_BAK_);

                                        /* Free pointer */
                                        free(uszRead_Total_Buff);
                                        free(uszWriteBuff_Record);
                                        free(uszWriteBuff_Org);

                                        return (VS_ERROR);
                                }
                        }
                        /* 剩餘長度小於1024 */
                        else if (lnReadLength < 1024)
                        {
                                /* 就只讀剩餘長度 */
                                if (inFILE_Read(&uldat_Handle, &uszRead_Total_Buff[1024 * i], lnReadLength) == VS_SUCCESS)
                                {
                                        break;
                                }
                                /* 讀失敗時 */
                                else
                                {
                                        /* Close檔案 */
                                        inFILE_Close(&uldat_Handle);
                                        inFILE_Close(&ulbak_Handle);
                                        inFILE_Delete((unsigned char *)_PIT_FILE_NAME_BAK_);

                                        /* Free pointer */
                                        free(uszRead_Total_Buff);
                                        free(uszWriteBuff_Record);
                                        free(uszWriteBuff_Org);

                                        return (VS_ERROR);
                                }
                        }
                } /* end for loop */
        }
        else
        {
                /* Close檔案 */
                inFILE_Close(&uldat_Handle);
                inFILE_Close(&ulbak_Handle);
                inFILE_Delete((unsigned char *)_PIT_FILE_NAME_BAK_);

                /* Free pointer */
                free(uszRead_Total_Buff);
                free(uszWriteBuff_Record);
                free(uszWriteBuff_Org);

                /* Seek失敗，所以回傳Error */
                return (VS_ERROR);
        }

        /* 防呆 總record數量小於要存取inPITRec Return ERROR */
        /* 算總Record數 */
        for (i = 0; i < (lnPITLength + 1); i++)
        {
                /* 一個換行，record數就+1 */
                if (uszRead_Total_Buff[i] == 0x0A)
                        inPIT_Total_Rec++;
        }

        /* 防呆 總record數量小於要存取inPITRec Return ERROR */
        if ((inPITRec + 1) > inPIT_Total_Rec)
        {
                if (ginDebug == VS_TRUE)
                {
                        inLogPrintf(AT, "No data or Index ERROR");
                }

                /* 關檔 */
                inFILE_Close(&uldat_Handle);
                inFILE_Close(&ulbak_Handle);
                inFILE_Delete((unsigned char *)_PIT_FILE_NAME_BAK_);

                /* Free pointer */
                free(uszRead_Total_Buff);
                free(uszWriteBuff_Record);
                free(uszWriteBuff_Org);

                return (VS_ERROR);
        }

        /* 記錄ReadTotalBuffer的陣列位置 */
        inRecIndex = 0;
        /* 用inPITRec決定要先存幾筆Record到PIT.bak，ex:inPITRec = 4，就會先存0~3的record進去bak檔 */
        for (i = 0; i < inPITRec; i ++)
        {
                j = 0;

                while (1)
                {
                        /* 存Record */
                        uszWriteBuff_Org[j] = uszRead_Total_Buff[inRecIndex];
                        j++;

                        /* 每讀完一筆record，就把baffer中record的資料寫入bak */
                        if (uszRead_Total_Buff[inRecIndex] == 0x0A)
                        {
                                /* 為防止寫入位置錯誤，先移動到bak的檔案結尾 */
                                inRetVal = inFILE_Seek(ulbak_Handle, 0, _SEEK_END_);

                                if (inRetVal != VS_SUCCESS)
                                {
                                        /* 關檔 */
                                        inFILE_Close(&uldat_Handle);
                                        inFILE_Close(&ulbak_Handle);
                                        inFILE_Delete((unsigned char *)_PIT_FILE_NAME_BAK_);

                                        /* Free pointer */
                                        free(uszWriteBuff_Record);
                                        free(uszWriteBuff_Org);
                                        free(uszRead_Total_Buff);

                                        return (VS_ERROR);
                                }

                                /* 寫入bak檔案 */
                                inRetVal = inFILE_Write(&ulbak_Handle, uszWriteBuff_Org, j);

                                if (inRetVal != VS_SUCCESS)
                                {
                                        /* 關檔 */
                                        inFILE_Close(&uldat_Handle);
                                        inFILE_Close(&ulbak_Handle);
                                        inFILE_Delete((unsigned char *)_PIT_FILE_NAME_BAK_);

                                        /* Free pointer */
                                        free(uszWriteBuff_Record);
                                        free(uszWriteBuff_Org);
                                        free(uszRead_Total_Buff);

                                        return (VS_ERROR);
                                }

                                /* 清空buffer為下一筆寫入做準備 */
                                memset(uszWriteBuff_Org, 0x00, sizeof(uszWriteBuff_Org));
                                inRecIndex++;

                                break;
                        }

                        /* 讀下一個字元 */
                        inRecIndex++;
                }
        }

        /* 存組好的該PITRecord 到 PIT.bak */
        inRetVal = inFILE_Seek(ulbak_Handle, 0, _SEEK_END_);

        if (inRetVal != VS_SUCCESS)
        {
                /* 關檔 */
                inFILE_Close(&uldat_Handle);
                inFILE_Close(&ulbak_Handle);
                inFILE_Delete((unsigned char *)_PIT_FILE_NAME_BAK_);

                /* Free pointer */
                free(uszWriteBuff_Record);
                free(uszWriteBuff_Org);
                free(uszRead_Total_Buff);

                return (VS_ERROR);
        }

        /* 將組好的record寫入bak檔 */
        inRetVal = inFILE_Write(&ulbak_Handle, uszWriteBuff_Record, inPackCount);

        if (inRetVal != VS_SUCCESS)
        {
                /* 關檔 */
                inFILE_Close(&uldat_Handle);
                inFILE_Close(&ulbak_Handle);
                inFILE_Delete((unsigned char *)_PIT_FILE_NAME_BAK_);

                /* Free pointer */
                free(uszWriteBuff_Record);
                free(uszWriteBuff_Org);
                free(uszRead_Total_Buff);

                return (VS_ERROR);
        }

        /* 存剩下的原PIT.dat Record 到 PIT.bak */
        /* 計算已存幾筆Record 移動到正確陣列位置 */
        /* inRecIndex要指到剩下還沒存進bak檔的第一個記憶體位置 */
        inPITRec = inPITRec + 1;
        j = 0;
        inRecIndex = 0;

        for (i = 0; i < lnPITLength; i++)
        {
                if (uszRead_Total_Buff[i] == 0x0A)
                {
                        j++;

                        if (j == inPITRec)
                        {
                                inRecIndex = i + 1;
                                break;
                        }
                }
        }

        i = 0;
        memset(uszWriteBuff_Org, 0x00, sizeof(uszWriteBuff_Org));

        while (1)
        {
                /* 接續存原PIT.dat的Record */
                uszWriteBuff_Org[i] = uszRead_Total_Buff[inRecIndex];
                i++;

                if (uszRead_Total_Buff[inRecIndex] == 0x0A)
                {
                        /* 移動到bak的檔案結尾 */
                        inRetVal = inFILE_Seek(ulbak_Handle, 0, _SEEK_END_);

                        if (inRetVal != VS_SUCCESS)
                        {
                                /* 關檔 */
                                inFILE_Close(&uldat_Handle);
                                inFILE_Close(&ulbak_Handle);
                                inFILE_Delete((unsigned char *)_PIT_FILE_NAME_BAK_);

                                /* Free pointer */
                                free(uszWriteBuff_Record);
                                free(uszWriteBuff_Org);
                                free(uszRead_Total_Buff);

                                return (VS_ERROR);
                        }

                        inRetVal = inFILE_Write(&ulbak_Handle, uszWriteBuff_Org, i);

                        if (inRetVal != VS_SUCCESS)
                        {
                                /* 關檔 */
                                inFILE_Close(&uldat_Handle);
                                inFILE_Close(&ulbak_Handle);
                                inFILE_Delete((unsigned char *)_PIT_FILE_NAME_BAK_);

                                /* Free pointer */
                                free(uszWriteBuff_Record);
                                free(uszWriteBuff_Org);
                                free(uszRead_Total_Buff);

                                return (VS_ERROR);
                        }

                        memset(uszWriteBuff_Org, 0x00, sizeof(uszWriteBuff_Org));
                        i = 0;
                }
                else if (uszRead_Total_Buff[inRecIndex] == 0x00)
                {
                        /* 讀到0x00結束 */
                        break;
                }

                inRecIndex++;
        }

        /* 關檔 */
        inFILE_Close(&uldat_Handle);
        inFILE_Close(&ulbak_Handle);

        /* Free pointer */
        free(uszWriteBuff_Record);
        free(uszWriteBuff_Org);
        free(uszRead_Total_Buff);

        /* 刪除原PIT.dat */
        if (inFILE_Delete((unsigned char *)_PIT_FILE_NAME_) != VS_SUCCESS)
        {
                return (VS_ERROR);
        }

        /* 將PIT.bak改名字為PIT.dat取代原檔案 */
        if (inFILE_Rename((unsigned char *)_PIT_FILE_NAME_BAK_, (unsigned char *)_PIT_FILE_NAME_) != VS_SUCCESS)
        {
                return (VS_ERROR);
        }

        /* inSavePITRec() END */
        if (ginDebug == VS_TRUE)
        {
                memset(szErrorMsg, 0x00, sizeof(szErrorMsg));
                sprintf(szErrorMsg, "inSavePITRec(%d) END!!", inPITRec - 1);
                inLogPrintf(AT, szErrorMsg);
                inLogPrintf(AT, "----------------------------------------");
        }

        return(VS_SUCCESS);
}

/*
set和get等價於相反的操作
各欄位的set和get function
*/

/*
Function        :inGetPayItemIndex
Date&Time       :
Describe        :
*/
int inGetPayItemIndex(char* szPayItemIndex)
{
        /* 傳進的指標 不得為空  Rec中的值長度須為正數 小於規定最大值 */
        if (szPayItemIndex == NULL || strlen(srPITRec.szPayItemIndex) <= 0 || strlen(srPITRec.szPayItemIndex) > 2)
        {
                /* debug */
                if (ginDebug == VS_TRUE)
                {
                        char    szErrorMsg[_DEBUG_MESSAGE_SIZE_ + 1];   /* debug message */

                        inLogPrintf(AT, "inGetPayItemIndex() ERROR !!");
                        
			if (szPayItemIndex == NULL)
                        {
                                inLogPrintf(AT, "szPayItemIndex == NULL");
                        }
                        else
                        {
                                memset(szErrorMsg, 0x00, sizeof(szErrorMsg));
                                sprintf(szErrorMsg, "szPayItemIndex length = (%d)", (int)strlen(srPITRec.szPayItemIndex));
                                inLogPrintf(AT, szErrorMsg);
                        }
                }

                return (VS_ERROR);
        }
        memcpy(&szPayItemIndex[0], &srPITRec.szPayItemIndex[0], strlen(srPITRec.szPayItemIndex));

        return (VS_SUCCESS);
}

/*
Function        :inSetPayItemIndex
Date&Time       :
Describe        :
*/
int inSetPayItemIndex(char* szPayItemIndex)
{
        memset(srPITRec.szPayItemIndex, 0x00, sizeof(srPITRec.szPayItemIndex));
        /* 傳進的指標 不得為空  傳進的值長度需大於0 小於規定最大值 */
        if (szPayItemIndex == NULL || strlen(szPayItemIndex) <= 0 || strlen(szPayItemIndex) > 2)
        {
                /* debug */
                if (ginDebug == VS_TRUE)
                {
                        char    szErrorMsg[_DEBUG_MESSAGE_SIZE_ + 1];   /* debug message */

                        inLogPrintf(AT, "inSetPayItemIndex() ERROR !!");
                        if (szPayItemIndex == NULL)
                        {
                                inLogPrintf(AT, "szPayItemIndex == NULL");
                        }
                        else
                        {
                                memset(szErrorMsg, 0x00, sizeof(szErrorMsg));
                                sprintf(szErrorMsg, "szPayItemIndex length = (%d)", (int)strlen(szPayItemIndex));
                                inLogPrintf(AT, szErrorMsg);
                        }
                }

                return (VS_ERROR);
        }
        memcpy(&srPITRec.szPayItemIndex[0], &szPayItemIndex[0], strlen(szPayItemIndex));

        return (VS_SUCCESS);
}

/*
Function        :inGetPayItemKeyMap
Date&Time       :
Describe        :
*/
int inGetPayItemKeyMap(char* szPayItemKeyMap)
{
        /* 傳進的指標 不得為空  Rec中的值長度須為正數 小於規定最大值 */
        if (szPayItemKeyMap == NULL || strlen(srPITRec.szPayItemKeyMap) <= 0 || strlen(srPITRec.szPayItemKeyMap) > 2)
        {
                /* debug */
                if (ginDebug == VS_TRUE)
                {
                        char    szErrorMsg[_DEBUG_MESSAGE_SIZE_ + 1];   /* debug message */

                        inLogPrintf(AT, "inGetPayItemKeyMap() ERROR !!");

                        if (szPayItemKeyMap == NULL)
                        {
                                inLogPrintf(AT, "szPayItemKeyMap == NULL");
                        }
                        else
                        {
                                memset(szErrorMsg, 0x00, sizeof(szErrorMsg));
                                sprintf(szErrorMsg, "szPayItemKeyMap length = (%d)", (int)strlen(srPITRec.szPayItemKeyMap));
                                inLogPrintf(AT, szErrorMsg);
                        }
                }

                return (VS_ERROR);
        }
        memcpy(&szPayItemKeyMap[0], &srPITRec.szPayItemKeyMap[0], strlen(srPITRec.szPayItemKeyMap));

        return (VS_SUCCESS);
}

/*
Function        :inSetPayItemKeyMap
Date&Time       :
Describe        :
*/
int inSetPayItemKeyMap(char* szPayItemKeyMap)
{
        memset(srPITRec.szPayItemKeyMap, 0x00, sizeof(srPITRec.szPayItemKeyMap));
        /* 傳進的指標 不得為空  傳進的值長度須為正數 小於規定最大值 */
        if (szPayItemKeyMap == NULL || strlen(szPayItemKeyMap) <= 0 || strlen(szPayItemKeyMap) > 2)
        {
                /* debug */
                if (ginDebug == VS_TRUE)
                {
                        char    szErrorMsg[_DEBUG_MESSAGE_SIZE_ + 1];   /* debug message */

                        inLogPrintf(AT, "inSetPayItemKeyMap() ERROR !!");

                        if (szPayItemKeyMap == NULL)
                        {
                                inLogPrintf(AT, "szPayItemKeyMap == NULL");
                        }
                        else
                        {
                                memset(szErrorMsg, 0x00, sizeof(szErrorMsg));
                                sprintf(szErrorMsg, "szPayItemKeyMap length = (%d)", (int)strlen(szPayItemKeyMap));
                                inLogPrintf(AT, szErrorMsg);
                        }
                }

                return (VS_ERROR);
        }
        memcpy(&srPITRec.szPayItemKeyMap[0], &szPayItemKeyMap[0], strlen(szPayItemKeyMap));

        return (VS_SUCCESS);
}

/*
Function        :inGetPayItemCode
Date&Time       :
Describe        :
*/
int inGetPayItemCode(char* szPayItemCode)
{
        /* 傳進的指標 不得為空  Rec中的值長度須為正數 小於規定最大值 */
        if (szPayItemCode == NULL || strlen(srPITRec.szPayItemCode) <= 0 || strlen(srPITRec.szPayItemCode) > 5)
        {
                /* debug */
                if (ginDebug == VS_TRUE)
                {
                        char    szErrorMsg[_DEBUG_MESSAGE_SIZE_ + 1];   /* debug message */

                        inLogPrintf(AT, "inGetPayItemCode() ERROR !!");

                        if (szPayItemCode == NULL)
                        {
                                inLogPrintf(AT, "szPayItemCode == NULL");
                        }
                        else
                        {
                                memset(szErrorMsg, 0x00, sizeof(szErrorMsg));
                                sprintf(szErrorMsg, "szPayItemCode length = (%d)", (int)strlen(srPITRec.szPayItemCode));
                                inLogPrintf(AT, szErrorMsg);
                        }
                }

                return (VS_ERROR);
        }
        memcpy(&szPayItemCode[0], &srPITRec.szPayItemCode[0], strlen(srPITRec.szPayItemCode));

        return (VS_SUCCESS);
}

/*
Function        :inSetPayItemCode
Date&Time       :
Describe        :
*/
int inSetPayItemCode(char* szPayItemCode)
{
        memset(srPITRec.szPayItemCode, 0x00, sizeof(srPITRec.szPayItemCode));
        /* 傳進的指標 不得為空  傳進的值長度須為正數 小於規定最大值 */
        if (szPayItemCode == NULL || strlen(szPayItemCode) <= 0 || strlen(szPayItemCode) > 5)
        {
                /* debug */
                if (ginDebug == VS_TRUE)
                {
                        char    szErrorMsg[_DEBUG_MESSAGE_SIZE_ + 1];   /* debug message */

                        inLogPrintf(AT, "inSetPayItemCode() ERROR !!");

                        if (szPayItemCode == NULL)
                        {
                                inLogPrintf(AT, "szPayItemCode == NULL");
                        }
                        else
                        {
                                memset(szErrorMsg, 0x00, sizeof(szErrorMsg));
                                sprintf(szErrorMsg, "szPayItemCode length = (%d)", (int)strlen(szPayItemCode));
                                inLogPrintf(AT, szErrorMsg);
                        }
                }

                return (VS_ERROR);
        }
        memcpy(&srPITRec.szPayItemCode[0], &szPayItemCode[0], strlen(szPayItemCode));

        return (VS_SUCCESS);
}

/*
Function        :inGetPayItemScript
Date&Time       :
Describe        :
*/
int inGetPayItemScript(char* szPayItemScript)
{
        /* 傳進的指標 不得為空  Rec中的值長度須為正數 小於規定最大值 */
        if (szPayItemScript == NULL || strlen(srPITRec.szPayItemScript) <= 0 || strlen(srPITRec.szPayItemScript) > 40)
        {
                /* debug */
                if (ginDebug == VS_TRUE)
                {
                        char    szErrorMsg[_DEBUG_MESSAGE_SIZE_ + 1];   /* debug message */

                        inLogPrintf(AT, "inGetPayItemScript() ERROR !!");

                        if (szPayItemScript == NULL)
                        {
                                inLogPrintf(AT, "szPayItemScript == NULL");
                        }
                        else
                        {
                                memset(szErrorMsg, 0x00, sizeof(szErrorMsg));
                                sprintf(szErrorMsg, "szPayItemScript length = (%d)", (int)strlen(srPITRec.szPayItemScript));
                                inLogPrintf(AT, szErrorMsg);
                        }
                }

                return (VS_ERROR);
        }
        memcpy(&szPayItemScript[0], &srPITRec.szPayItemScript[0], strlen(srPITRec.szPayItemScript));

        return (VS_SUCCESS);
}

/*
Function        :inSetPayItemScript
Date&Time       :
Describe        :
*/
int inSetPayItemScript(char* szPayItemScript)
{
        memset(srPITRec.szPayItemScript, 0x00, sizeof(srPITRec.szPayItemScript));
        /* 傳進的指標 不得為空  傳進的值長度須為正數 小於規定最大值 */
        if (szPayItemScript == NULL || strlen(szPayItemScript) <= 0 || strlen(szPayItemScript) > 40)
        {
                /* debug */
                if (ginDebug == VS_TRUE)
                {
                        char    szErrorMsg[_DEBUG_MESSAGE_SIZE_ + 1];   /* debug message */

                        inLogPrintf(AT, "inSetPayItemScript() ERROR !!");

                        if (szPayItemScript == NULL)
                        {
                                inLogPrintf(AT, "szPayItemScript == NULL");
                        }
                        else
                        {
                                memset(szErrorMsg, 0x00, sizeof(szErrorMsg));
                                sprintf(szErrorMsg, "szPayItemScript length = (%d)", (int)strlen(szPayItemScript));
                                inLogPrintf(AT, szErrorMsg);
                        }
                }

                return (VS_ERROR);
        }
        memcpy(&srPITRec.szPayItemScript[0], &szPayItemScript[0], strlen(szPayItemScript));

        return (VS_SUCCESS);
}
